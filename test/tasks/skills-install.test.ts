...
describe("skillsInstallTask", () => {
  ...
  it("dryRun includes vue and frontend skills for vue projects", async () => {
    ...
    const diffs = await skillsInstallTask.dryRun(path.join(fixtures, "vue-vite"), profile);
    expect(diffs.length).toBe(1);
    const after = diffs[0].after ?? "";
    // Vue skills
  ...
  it("dryRun includes nextjs skills for nextjs projects", async () => {
    ...
    const diffs = await skillsInstallTask.dryRun(path.join(fixtures, "nextjs"), profile);
    expect(diffs.length).toBe(1);
    const after = diffs[0].after ?? "";
    // Next.js skills
  ...
  it("dryRun includes antd skill for projects with antd", async () => {
    const profile = await detectProject(path.join(fixtures, "react-ui-libraries"));
    const diffs = await skillsInstallTask.dryRun(
  ...
  it("dryRun includes heroui-native skill for react-native projects with heroui-native", async () => {
    const profile = await detectProject(path.join(fixtures, "react-native-hero"));
    const diffs = await skillsInstallTask.dryRun(path.join(fixtures, "react-native-hero"), profile);
    expect(diffs.length).toBe(1);
    const after = diffs[0].after ?? "";
  ...
  it("returns skip when no relevant framework is detected", async () => {
    ...
    const status = await skillsInstallTask.check(path.join(fixtures, "node-only"), profile);
    expect(status).toBe("skip");
  });

  it("returns empty dryRun when no relevant framework is detected", async () => {
    const profile = await detectProject(path.join(fixtures, "node-only"));
    const diffs = await skillsInstallTask.dryRun(path.join(fixtures, "node-only"), profile);
