...
| "What templates are available?" | Uses the template catalog to explain the 5 valid IDs |
...
- Scaffold and preview commands with all CLI flags, including `--template`, `--pm`, `--no-git`, `--clean`, `--force`, `--ref`, `--yes`, `--quiet`, and `--json`
