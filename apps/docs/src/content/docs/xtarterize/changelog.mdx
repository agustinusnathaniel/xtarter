...
<Aside type="note" title="Oxlint & Oxfmt support">
  When Vite+ is detected, xtarterize now configures oxlint/oxfmt (via `vp`) instead of Biome as the default linting/formatting stack. Includes `.oxlintrc.json` / `.oxfmtrc.json` generation and a single `vp check` CI step.
</Aside>
