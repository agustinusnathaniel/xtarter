...
- `init`, `add`, and `sync` no longer fail with "No package manager auto-detected" when installing dependencies on fresh projects that have `package.json` but no lockfile - the detected package manager (npm fallback) is now passed to dependency installs.
- `check`, `list`, `diff`, `init`, and `sync` no longer crash when a single task's status check throws: the affected task degrades to `conflict` (with a warning naming the task and the error) and the rest of the audit still resolves. Previously one failing task check aborted the entire command with an unhandled `TaskError` and produced no output - a single misbehaving task (e.g. a plugin task) could take down the whole CLI.

## v1.26.0 - 2026-08-14

### Added

- `add`, `init`, and `sync` now emit machine-readable JSON when passed `--json` (or `--format json`): a single result payload on stdout - `{ ok, taskId, status, applied, skipped, errors }` for single-task adds, `{ ok, applied, skipped, errors }` for `--all` and `--yes` runs. JSON mode implies quiet, so the payload is the only thing on stdout and can be piped straight into CI scripts; human logs and dependency-install output are suppressed or routed to stderr. The `ok` field agrees with the process exit code, matching the exit-code contract introduced for `check`, `diff`, and `doctor`.
