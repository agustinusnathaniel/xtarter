...
With `--verbose`, a **System** group adds platform, OS release, architecture,
CPU count, and RAM. Diagnostics also support `--json`.
...
<AllPackageManagers type="exec" pkg="xtarterize@latest" args="undo --json" title="JSON output" />
...
<AllPackageManagers type="exec" pkg="xtarterize@latest" args="restore tsconfig.json --json" title="JSON output" />
