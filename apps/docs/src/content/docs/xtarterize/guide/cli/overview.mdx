...
| `--quiet` | all except `query` | Suppress prompts and verbose output; `init` and `sync` use the explicit non-interactive apply path, while `restore` still prompts for a backup unless `--yes` |
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="doctor" />
...
| Option | Description |
| --- | --- |
| `--dry-run` | Preview changes without applying or creating backups; the project `.gitignore` may still be updated |
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="sync" />

## `diff`

Preview changes for `new`, `patch`, and `conflict` tasks without writing conformance files or creating backups; the project `.gitignore` may still be created or updated. New files are shown in full; patched files use unified diffs. Changes to the same JSON file are merged into one diff.

Options: `--format <terminal|json>`.

<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="diff" />
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="check" />
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="add lint/biome" title="Specific task" />
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="undo" />
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args="restore tsconfig.json" />
...
<AllPackageManagers type="dlx" pkg="xtarterize@latest" args='query "strict typescript"' />
...
Before modifying a file, xtarterize writes a timestamped backup under `.xtarterize/backups/` and records the run in `last-run.json`. The session may add `/.xtarterize/` to `.gitignore` when filesystem access succeeds. Use `undo` to revert a whole run, or `restore <file>` for one file.
