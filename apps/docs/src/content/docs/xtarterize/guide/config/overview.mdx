...
- **Monorepo** - Detected via [`pnpm-workspace.yaml`](https://pnpm.io/pnpm-workspace_yaml), [`turbo.json`](https://turbo.build/repo/docs/reference/configuration), [`nx.json`](https://nx.dev/reference/nx-json), [`lerna.json`](https://lerna.js.org/docs/configuration), or at least two of the `packages/`, `apps/`, `services/` directories
...
Plugin tasks may implement the original Promise-based `Task` contract or return Effects. Both shapes are normalized by `toTaskEffect()`, so existing plugins keep working and new plugins can use Effect services like `ProcessRunner` (ADR 036).
