...
Most tasks are created through factory functions in [`factory/index.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/index.ts) that eliminate boilerplate:
...
- **`createJsonMergeTask`** - For tasks that deep-merge JSON configs ([`tsconfig.json`](https://www.typescriptlang.org/tsconfig/), [`biome.json`](https://biomejs.dev/reference/configuration/), [`turbo.json`](https://turbo.build/repo/docs/reference/configuration)). Uses shared JSON mutation helpers in [`factory/json-config.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/json-config.ts)
- **`createMultiFileJsonMergeTask`** - For tasks that merge multiple JSON files ([`.vscode/settings.json`](https://code.visualstudio.com/docs/getstarted/settings#_settingsjson) + [`.vscode/extensions.json`](https://code.visualstudio.com/docs/editor/extension-marketplace#_workspace-recommended-extensions)). Supports custom per-file merge functions and reuses [`factory/json-config.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/json-config.ts)
...
All factory option types accept an optional `scope` parameter, which
is passed through to the constructed `Task`. For example:

```typescript
createFileTask({
  id: 'my-task',
  label: 'My Task',
  scope: 'root',  // only offered at monorepo root
  // ... other options
})
```
...
- [`factory/json-config.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/json-config.ts) - centralized JSON config `check`/`dryRun` logic
...
- [`factory/equivalence.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/equivalence.ts) - shared equivalence detection helpers (`deepEqual`, `normalizeExtends`, `hasScriptValue`)
