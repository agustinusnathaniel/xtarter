...
- **`createFileTask`** — For tasks that write a single file (CI workflows, [renovate](https://docs.renovatebot.com/), [commitlint](https://commitlint.js.org/), [knip](https://knip.dev/), [`.editorconfig`](https://editorconfig.org/), [`.gitignore`](https://git-scm.com/docs/gitignore), AI skills). Supports exact-match comparison with `normalizeLineEndings` and optional custom `checkFn`.
