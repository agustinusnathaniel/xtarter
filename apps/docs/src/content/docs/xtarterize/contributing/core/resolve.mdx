...
When the project is a monorepo, `resolveTasks()` additionally filters by each task's `scope` (`'root' | 'package' | 'both'`, default `'both'`): package-scoped tasks are excluded at the monorepo root, and root-scoped tasks are excluded inside a workspace package. Non-monorepo projects skip scope filtering entirely. See the [Monorepo guide](/xtarterize/guide/monorepo/) for the user-facing rules.
