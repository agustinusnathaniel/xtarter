...
<Aside type="note">
  When both `react` and `react-native` (or `expo`) are present in dependencies, `detectFramework()` returns `'react-native'`; there is no prompt to disambiguate. `framework` is `null` only when the project has no `package.json`.
</Aside>
