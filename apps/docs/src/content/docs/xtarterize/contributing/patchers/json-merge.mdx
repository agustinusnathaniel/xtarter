...
`jsonMerge` targets in `@xtarterize/tasks` use both functions: `mergeJson`
computes the target state, then `patchJson` applies it surgically to the
original text. This shared behavior lives in
[`factory/targets.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/targets.ts).
