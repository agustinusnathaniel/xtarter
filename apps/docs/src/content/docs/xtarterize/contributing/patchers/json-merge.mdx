...
Tasks that need to combine arrays, such as VS Code extension
recommendations, use a custom merge function instead of `mergeJson`.
...
`createJsonMergeTask` and `createMultiFileJsonMergeTask` use both functions:
`mergeJson` computes the target state, then `patchJson` applies it surgically to
the original text. In `@xtarterize/tasks`, this shared behavior is centralized
in [`factory/json-config.ts`](https://github.com/agustinusnathaniel/xtarterize/blob/main/packages/tasks/src/factory/json-config.ts).
