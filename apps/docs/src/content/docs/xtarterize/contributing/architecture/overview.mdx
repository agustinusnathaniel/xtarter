...
sequenceDiagram
    ...
    participant Runtime as runtime.ts (runCliProgram)
    ...
    CLI->>Runtime: runCliProgram(initProgram)
    Runtime->>Runtime: provide AppLayer (DepsInstaller + ProcessRunner + Prompter)
    Runtime->>Session: CommandSession.open(args)
    Session->>Core: ensureXtarterizeGitignore(cwd) [plain leaf, lifted]
    Session->>Core: runPreflight(cwd) [plain leaf, lifted]
    Session->>Core: detectProject + resolveProjectTasks (Effect)
    Core-->>Session: ProjectProfile + statuses + timing
    Session->>Core: planTasks (Effect)
    Core-->>Session: ApplyPlan (diffs, dependencies, files)
    Session->>User: displayPlan + confirm via Prompter service
    User->>Session: Confirm
    Session->>Core: executePlan (Effect)
    Core->>Core: backupFile + writeRunManifest [plain leaves, lifted]
    Core->>Core: DepsInstaller.install(dependencies)
    Core->>Tasks: task.check/dryRun/apply (Effect or Promise, normalized)
    Tasks->>Patchers: mergeJson / patchJson / injectVitePluginIntoCode
    Patchers-->>Tasks: Modified content
    Tasks-->>Core: Result or TaskError
    Core-->>Session: ApplyResult
    Session->>User: report outcome (terminal or JSON)
    Runtime->>Runtime: Exit/Cause -> exit code (interrupt-only = 0)
```
...
- **One command session** - `CommandSession` owns open, plan, execute, and report; commands declare selection policy and output shape, and prompts are injected through the `Prompter` service (clack in production, scripted in tests)
...
- **Packages return Effects, apps run them** - `packages/core` and `packages/tasks` construct `Effect` values; `apps/xtarterize/src/runtime.ts` is the single runtime edge (`runCliProgram` with `AppLayer`). Pure and Promise leaves (detection, filesystem, backup, preflight) stay plain and are lifted at the nearest seam (ADR 036)
- **Effect services own the effectful edges** - `ProcessRunner` (tinyexec with interruption and timeout), `DepsInstaller` (batched installs), and `Prompter` (interactive prompts) are Effect services provided at the runtime edge
