import { enhanceDiff, type FileDiff, isJsonFile } from '@xtarterize/core';
import { patchJson } from '@xtarterize/patchers';

export function mergeFileDiffs(diffs: Array<FileDiff>): Array<FileDiff> {
  const grouped = new Map<string, Array<FileDiff>>();
  for (const diff of diffs) {
    const list = grouped.get(diff.filepath) ?? [];
    list.push(diff);
    grouped.set(diff.filepath, list);
  }

  const merged: Array<FileDiff> = [];
  for (const [filepath, list] of grouped) {
    let result: FileDiff;

    if (list.length === 1) {
      result = list[0];
    } else if (isJsonFile(filepath)) {
      const first = list.find((d) => d.before !== null);
      const before = first?.before ?? list[0].before;
      let after = before ?? '{}';
      for (const diff of list) {
        try {
          after = patchJson(after, JSON.parse(diff.after));
        } catch {
          after = diff.after;
        }
      }
      result = { after, before, filepath };
    } else {
      // For non-JSON files with multiple diffs, concatenate all diffs
      // Use the first task's before as the base, last task's after as final
      result = {
        after: list[list.length - 1].after,
        before: list[0].before,
        filepath,
      };
    }

    merged.push(enhanceDiff(result));
  }

  return merged;
}
