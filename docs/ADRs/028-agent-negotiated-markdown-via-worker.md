...
- **(a) Cloudflare zone feature "Markdown for Agents" (ADR-027).** Zero repository code, but requires a Pro plan or higher ($20+/mo) and an external toggle outside version control; still the preferred simplification if the plan is upgraded.
- **(b) Static `_headers` with `Vary` only.** Fixes the header half of the audit without a worker, but does not negotiate `Accept: text/markdown` on the same URL or provide a Markdown 404 body.
- **(c) Turndown-based worker converting HTML to Markdown at request time.** Per-page twins already cover the content corpus, so runtime conversion would add complexity, latency, and fidelity risk for no measurable benefit.
- **(d) `negotiator` (1.1.0) / `accepts` (1.3.8) for `wantsMarkdown` (22 lines custom).** Full RFC 7231 negotiation would treat `*/*` and `text/*` as matching `text/markdown`, incorrectly serving Markdown to browsers on navigation, and `negotiator` needs a bundling step for Workers. Verdict: KEEP CUSTOM (narrower, zero deps, Workers-compatible, smaller).
- **(e) `vary` (1.1.2) for `mergeVary` (30 lines custom).** Node `ServerResponse` style; needs a Headers-API adapter and still does not guarantee the deterministic sorted `Vary` the audit requires. Verdict: KEEP CUSTOM.
- **(f) `starlight-dot-md` (0.2.1) / `starlight-md-txt` (0.1.0) for `.md` twins and probing.** Neither implements same-URL negotiation, `Vary` merging, or Markdown 404 recovery; swapping would lose page-action buttons and not remove the worker. Verdict: KEEP `starlight-page-actions` + `starlight-llms-txt`, KEEP CUSTOM probe loop.
- **(g) `vite-plugin-static-copy` (4.1.1) / `astro:build:done` integration for the worker copy.** Adds config and 10-20 lines over the one-line, idempotent `cp` in `apps/docs/package.json:build`; the docs build does not run on Windows in CI. Verdict: KEEP `cp`.

Ecosystem review (2026-08-25, branch `feat/agentic-worker-1`, catalog `starlight-page-actions@0.7.1` + `starlight-llms-txt@0.11.0` + `astro@7.2.4`): no replacement was strictly better on zero-deps Workers compatibility, narrow `text/markdown`-only matching, `q=0` correctness, deterministic `Vary` sorting, and bundle size. Re-check if Workers gain first-class npm bundling or a Starlight plugin ships same-URL negotiation.
