...
3. **The plugin system (plan 019) is a better direction.** If templates need to live outside the npm package, they should be distributed as `Task` objects (TypeScript source with logic), not static files. The registry *concept* (GitHub-hosted, ref-pinned) is useful for that. The shadcn CLI itself is not.
