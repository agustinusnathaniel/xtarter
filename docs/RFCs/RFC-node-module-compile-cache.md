...
| Pro | Con |
| --- | --- |
| CI runs frequently, so savings compound | Out of scope — xtarterize configures *linting and type checking*, not Node runtime settings |
...
- [ ] Consider adding `NODE_COMPILE_CACHE=1` to generated CI workflow templates as a best practice for the *projects* xtarterize configures (separate decision, tracked independently)
