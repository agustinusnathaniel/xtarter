...
export function renderSteps(steps: Array<YamlStep>, indent: number): string {
  if (steps.length === 0) {
    return '';
  }
  ...
  return steps
    .map((step) => {
      const pairs: Array<[string, string | boolean | Record<string, string>]> =
        [];
